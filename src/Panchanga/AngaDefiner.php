...
class AngaDefiner
{
    ...
    public function getNakshatra($withLimit = false, $withAbhijit = false, $grahaKey = Graha::KEY_CH)
    {
        ...
        if ($currentNakshatra = Nakshatra::getInstance($nakshatra['key'])) {
            $nakshatra['type'] = $currentNakshatra->nakshatraType;
            $nakshatra['ruler'] = $currentNakshatra->nakshatraRuler;
            $nakshatra['deva'] = $currentNakshatra->nakshatraDeva;
            $nakshatra['left_dms'] = DDtoDMS($nakshatra['left']);
        }
